...
gen-check: gen ## Fail if generated files are stale
	git diff --exit-code -- frontend/src/generated frontend/public/openapi.json || (git diff -- frontend/src/generated frontend/public/openapi.json; echo "::error::generated openapi hash=$$(sha256sum frontend/public/openapi.json | cut -d' ' -f1), tracked hash=$$(git show HEAD:frontend/public/openapi.json | sha256sum | cut -d' ' -f1)"; git diff --numstat -- frontend/src/generated frontend/public/openapi.json; exit 1)
